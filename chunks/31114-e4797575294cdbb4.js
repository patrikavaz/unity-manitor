        ...
        86898: (e, t, n) => {
            ...
            v.prototype.eos = function() {
                ...
                return void 0 == s && (s = function(e, t) {
                    ...
                    S(t || b.tags);
                    for (var M = new v(e); !M.eos();) {
                        if (s = M.pos, y = M.scanUntil(n))
                            for (var A = 0, O = y.length; A < O; ++A) ! function(e) {
                                return !u.call(c, e)
                            }(w = y.charAt(A)) ? (j = !0, C = !0, U += " ") : (_.push(x.length), U += w), x.push(["text", w, s, s + 1]), s += 1, "\n" === w && (I(), U = "", P = 0, C = !1);
                        if (!M.scan(n)) break;
                        if (T = !0, l = M.scan(g) || "name", M.scan(p), "=" === l ? (y = M.scanUntil(h), M.scan(h), M.scanUntil(r)) : "{" === l ? (y = M.scanUntil(o), M.scan(d), M.scanUntil(r), l = "&") : y = M.scanUntil(r), !M.scan(r)) throw Error("Unclosed tag at " + M.pos);
                        if (k = ">" == l ? [l, y, s, M.pos, U, P, C] : [l, y, s, M.pos], P++, x.push(k), "#" === l || "^" === l) E.push(k);
                        else if ("/" === l) {
                            if (!(m = E.pop())) throw Error('Unopened section "' + y + '" at ' + s);
                            if (m[1] !== y) throw Error('Unclosed section "' + m[1] + '" at ' + s)
                        } else "name" === l || "{" === l || "&" === l ? j = !0 : "=" === l && S(y)
                    }
                    if (I(), m = E.pop()) throw Error('Unclosed section "' + m[1] + '" at ' + M.pos);
                    return function(e) {
